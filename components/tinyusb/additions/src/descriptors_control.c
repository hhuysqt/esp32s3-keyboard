    ...
    HID_COLLECTION ( HID_COLLECTION_PHYSICAL   )                   ,\
      ...
      HID_USAGE_PAGE  ( HID_USAGE_PAGE_CONSUMER ), \
       /* Horizontal wheel scroll [-127, 127] */ \
        ...
        HID_INPUT       ( HID_DATA | HID_VARIABLE | HID_RELATIVE ), \
      HID_USAGE_PAGE  ( HID_USAGE_PAGE_DESKTOP )                   ,\
        /* Resolution multipler for high-resolution mouse*/ \
...
uint8_t const desc_hid_report[] = {
    ...
    MY_HID_REPORT_DESC_MOUSE(HID_REPORT_ID(REPORT_ID_MOUSE)),
...
{
    ESP_LOGD("TUSB", "%s(%u)", __func__, index);
...
{
    (void) langid;
    ESP_LOGD("TUSB", "%s(%u, %u)", __func__, index, langid);
...
{
    ESP_LOGD("TUSB", "%s(%u)", __func__, itf);
    (void)itf;
